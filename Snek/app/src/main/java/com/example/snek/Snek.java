...
public class Snek {
    ...
    private ArrayList<SnekPart> arrSnekPart = new ArrayList<>();
    private int length;
    private boolean move_left, move_right, move_up, move_down;

    public Snek(Bitmap bm, int x, int y, int length) {
        this.bm = bm;
        this.length = length;
        ...
        setMove_right(true);
        arrSnekPart.add(new SnekPart(bm_head_right, x, y));
        for (int i = 1; i < length-1; i++){
            this.arrSnekPart.add(new SnekPart(bm_body_horizontal, this.arrSnekPart.get(i-1).getX()-GameView.sizeElementMap, y));
        }
        arrSnekPart.add(new SnekPart(bm_tail_right, arrSnekPart.get(length-2).getX()-GameView.sizeElementMap, arrSnekPart.get(length-2).getY()));
    }

    public void update(){
        for(int i = length-1; i > 0; i--){
            arrSnekPart.get(i).setX(arrSnekPart.get(i-1).getX());
            arrSnekPart.get(i).setY(arrSnekPart.get(i-1).getY());
        }
        if(move_right){
            arrSnekPart.get(0).setX(arrSnekPart.get(0).getX()+GameView.sizeElementMap);
            arrSnekPart.get(0).setBm(bm_head_right);
        }else if(move_down){
            arrSnekPart.get(0).setY(arrSnekPart.get(0).getY()+GameView.sizeElementMap);
            arrSnekPart.get(0).setBm(bm_head_down);
        }else if(move_up){
            arrSnekPart.get(0).setY(arrSnekPart.get(0).getY()-GameView.sizeElementMap);
            arrSnekPart.get(0).setBm(bm_head_up);
        }else{
            arrSnekPart.get(0).setX(arrSnekPart.get(0).getX()-GameView.sizeElementMap);
            arrSnekPart.get(0).setBm(bm_head_left);
        }
        for (int i = 1; i < length - 1; i++){
            if(arrSnekPart.get(i).getrLeft().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrBottom().intersect(arrSnekPart.get(i-1).getrBody())
                    || arrSnekPart.get(i).getrBottom().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrLeft().intersect(arrSnekPart.get(i-1).getrBody())){
                arrSnekPart.get(i).setBm(bm_body_bottom_left);
            }else if (arrSnekPart.get(i).getrLeft().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrTop().intersect(arrSnekPart.get(i-1).getrBody())
                    || arrSnekPart.get(i).getrLeft().intersect(arrSnekPart.get(i-1).getrBody())
                    && arrSnekPart.get(i).getrTop().intersect(arrSnekPart.get(i+1).getrBody())){
                arrSnekPart.get(i).setBm(bm_body_top_left);
            }else if (arrSnekPart.get(i).getrRight().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrTop().intersect(arrSnekPart.get(i-1).getrBody())
                    || arrSnekPart.get(i).getrRight().intersect(arrSnekPart.get(i-1).getrBody())
                    && arrSnekPart.get(i).getrTop().intersect(arrSnekPart.get(i+1).getrBody())) {
                arrSnekPart.get(i).setBm(bm_body_top_right);
            }else if(arrSnekPart.get(i).getrRight().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrBottom().intersect(arrSnekPart.get(i-1).getrBody())
                    || arrSnekPart.get(i).getrRight().intersect(arrSnekPart.get(i-1).getrBody())
                    && arrSnekPart.get(i).getrBottom().intersect(arrSnekPart.get(i+1).getrBody())){
                arrSnekPart.get(i).setBm(bm_body_bottom_right);
            }else if(arrSnekPart.get(i).getrLeft().intersect(arrSnekPart.get(i-1).getrBody())
                    && arrSnekPart.get(i).getrRight().intersect(arrSnekPart.get(i+1).getrBody())
                    || arrSnekPart.get(i).getrLeft().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrRight().intersect(arrSnekPart.get(i-1).getrBody())){
                arrSnekPart.get(i).setBm(bm_body_horizontal);
            }else if(arrSnekPart.get(i).getrTop().intersect(arrSnekPart.get(i-1).getrBody())
                    && arrSnekPart.get(i).getrBottom().intersect(arrSnekPart.get(i+1).getrBody())
                    || arrSnekPart.get(i).getrTop().intersect(arrSnekPart.get(i+1).getrBody())
                    && arrSnekPart.get(i).getrBottom().intersect(arrSnekPart.get(i-1).getrBody())){
                arrSnekPart.get(i).setBm(bm_body_vertical);
            }else{
                if(move_right){
                    arrSnekPart.get(i).setBm(bm_body_horizontal);
                }else if(move_down){
        ...
        if(arrSnekPart.get(length-1).getrRight().intersect(arrSnekPart.get(length-2).getrBody())){
            arrSnekPart.get(length-1).setBm(bm_tail_right);
        }else if(arrSnekPart.get(length-1).getrLeft().intersect(arrSnekPart.get(length-2).getrBody())){
            arrSnekPart.get(length-1).setBm(bm_tail_left);
        }else if(arrSnekPart.get(length-1).getrBottom().intersect(arrSnekPart.get(length-2).getrBody())){
            arrSnekPart.get(length-1).setBm(bm_tail_down);
        }else{
            arrSnekPart.get(length-1).setBm(bm_tail_up);
        }
    }

    public void drawSnake(Canvas canvas){
        for(int i = length-1; i >= 0; i--){
            canvas.drawBitmap(arrSnekPart.get(i).getBm(), arrSnekPart.get(i).getX(), arrSnekPart.get(i).getY(), null);
        }
    }
    ...
    public ArrayList<SnekPart> getArrSnekPart() {
        return arrSnekPart;
    }

    public void setArrSnekPart(ArrayList<SnekPart> arrSnekPart) {
        this.arrSnekPart = arrSnekPart;
    }
    ...
    public void addPart() {
        SnekPart p = this.arrSnekPart.get(length-1);
        this.length += 1;
        if(p.getBm()==bm_tail_right){
            this.arrSnekPart.add(new SnekPart(bm_tail_right, p.getX()-GameView.sizeElementMap, p.getY()));
        }else if(p.getBm()==bm_tail_left){
            this.arrSnekPart.add(new SnekPart(bm_tail_left, p.getX()+GameView.sizeElementMap, p.getY()));
        }else if(p.getBm()==bm_tail_up){
            this.arrSnekPart.add(new SnekPart(bm_tail_up, p.getX(), p.getY()+GameView.sizeElementMap));
